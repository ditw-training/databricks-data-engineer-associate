...
| `lakeflow_metadata_driven/` | **Python, dynamic** (config table `pipeline_config` → one generic file emits the whole graph; factory functions, closure pitfalls) | RetailHub (`dataset/`) | **DEMO 3 in module 07** — metadata-driven ingestion framework pattern (*beyond exam*, flagged; homework / fast finishers in the 6-hour agenda) |
| `lakeflow_ny_demo/` | **Python** (`from pyspark import pipelines as dp`, `dp.create_auto_cdc_flow` — SCD1 vs SCD2 side by side) | `samples.tpch` | Spare instructor demo of the Python API — superseded by `lakeflow_step_by_step` for the SCD story; keep as backup (needs only `samples` catalog, no landing files) |

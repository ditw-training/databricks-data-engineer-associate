...
1. **Infrastructure:** `infra/terraform` → `terraform apply` (⚠️ coordinate timing — never during another training on the same subscription). Then run `infra/sql/enable_cdc.sql` against the AdventureWorksLT DB and replace the `lakeflow_connect` user's placeholder password.
2. **Workspace:** premium (14-day trial window covers the training). Verify: serverless notebooks enabled, Lakeflow Connect SQL Server connector available (UC + serverless enabled, permission to create classic clusters for the gateway), web terminal availability (needed for the primary path of LAB 09).

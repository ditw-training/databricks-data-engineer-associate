...
1. **Infrastructure:** `infra/terraform` → `terraform apply` (⚠️ coordinate timing — never during another training on the same subscription). Then run `infra/sql/create_login_master.sql` (against `master`, set the `lakeflow_connect` password first), `infra/sql/enable_cdc.sql` (against AdventureWorksLT) and the Databricks utility script + `lakeflowSetupChangeDataCapture` — see `infra/terraform/README.md`. Run `notebooks/setup/00_pre_config` — **incl. Step 4b (workspace access)**: the training group `alt_trn_gr` must be **assigned to the workspace (User)** in the Account console *and* have the entitlements **`workspace-access` + `databricks-sql-access`** (the built-in `users` group had none on the 2026-09 workspace — without them participants cannot open the workspace). Ask one participant to log in the day before.
...
- **lab_07 Step 4 copies `orders_stream_004.json` into `datasets/orders/stream/`:** module 05, lab_05 and BONUS_05 read `orders/stream/*.json` and expect 3 files — if someone re-runs lab_05 after lab_07, counts change. Run lab_05 before lab_07 (agenda order) or delete the copied file first.
- **Lakeflow demos (verified 2026-09-15):** DEMO 1 step 8 FAIL only happens with a *full refresh* of `silver_orders_checked` (streaming tables check only new rows); DEMO 3 reads `<folder>/*.<format>` with a strict globber (the dataset folders mix csv/xlsx/parquet and sub-folders); `lakeflow_ny_demo` needs `generate_customer_data.py` first and is slow (~750k customers).
